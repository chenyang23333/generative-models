...
def parse_prompt_attention(text):
    """
    parse a string with attention tokens and returns a list of pairs: text and its associated weight
    """
    raise NotImplementedError
...
def get_tokenize_functions(tokenizer, context_length, pad_with_eos=True):
    ...
    Returns:
        A tokenize function that handles the text prompts exceeding the context length by reorganizing it into chunks
    ...
    def get_prompt_token_ids(prompt: str):
        ...
        Args:
            prompt, string.
        Returns:
            token_ids, torch.Tensor, shape is (n_chunks, context_length)
        """
        if len(prompt) > 0:
            token_ids = tokenizer.encode(prompt)
        else:
            token_ids = [bos, eos]  # simple tokenizer cannot handle an empty string
        if token_ids[0] == bos:
            token_ids.pop(0)
        if token_ids[-1] == eos:
            token_ids.pop(-1)
        ...
        if len(token_ids) > 0 or len(new_token_ids) == 0:
            ...
            )  # diffuser seems to insert a padding token before eos
            new_token_ids.append(temp_token_ids)
        new_token_ids = torch.tensor(new_token_ids, dtype=torch.int64)  # (n_chunks, context_len)
    ...
    def get_prompt_token_ids_batch(prompts, return_tensor=True):
        ...
        return_tensor, bool. If False, the returned item will be a list of tensors, where each tensor has the shape (n_chunks, context_len).
           ...
           If `return_tensor=True`, it will force the number of chunks equals the maximum of n_chunks in this batch.
        ...
        if return_tensor:
            ...
            for token_ids in group_ids:
                if token_ids.shape[0] < max_len:
                    token_ids = torch.cat([token_ids] + [pad_token_ids] * (max_len - token_ids.shape[0]), dim=0)
                new_group_ids.append(token_ids)
            group_ids = torch.stack(new_group_ids, dim=0)
        return group_ids

    return get_prompt_token_ids, get_prompt_token_ids_batch

def get_text_embedding_functions(text_encoder_embedding_function):
    def get_prompt_text_embedding(token_ids: torch.Tensor):
        """
        Args:
...
def _force_n_chunks_token_ids(token_ids, pad_token_ids, force_n_chunks):
    if token_ids.size(0) < force_n_chunks:
        num = force_n_chunks - token_ids.size(0)
        token_ids = torch.cat([token_ids] + [pad_token_ids] * num, dim=0)
    elif token_ids.size(0) > force_n_chunks:
...
def _text_embedder_tokenize_long_prompt(embedder, input_value, force_n_chunks=None, return_tensor=True):
    ...
    if force_n_chunks is not None:
        ...
        if return_tensor:
            emb_token = torch.stack(emb_token, dim=0)
    return emb_token
...
def embedding(model, *tokens, force_zero_embeddings=None):
    ...
    for i in range(len(model.embedders)):
        ...
        if is_text_encoder:
            _, get_text_embed_func = get_text_embedding_functions(embedder.forward)
        ...
        for emb in emb_out:
            if embedder.ucg_rate > 0.0 and embedder.legacy_ucg_val is None:
                emb = (
                    (1.0 - embedder.ucg_rate) * torch.ones_like(emb[:, 0:1], dtype=emb.dtype)
                    * emb
                )
            ...
            if emb.dim() == 2:  # vector
                if vector is None:
                    ...
                    vector = torch.cat((vector, emb), 1)
            elif emb.dim() == 3:  # crossattn
                if crossattn is None:
                    crossattn = emb
                else:
                    crossattn = torch.cat((crossattn, emb), 2)
            else:  # concat
                if concat is None:
                    concat = emb
                else:
                    concat = torch.cat((concat, emb), 1)
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                if T is None:
                    ...
                    pil_image = to_pil_image(grid.cpu())
                    ...
                    for i, vid in enumerate(as_vids):
                        ...
                        pil_image = to_pil_image(grid.cpu())
                        # Save the PIL Image as a .jpg file
                        pil_image.save(f"output_image_{i}.jpg")
                        st.image(
                            grid.cpu().numpy(),
                            f"Sample #{i} as image",
                        )
...
def do_sample(
    ...
    return_latents=False,
    filter=None,
    adapter_states: Optional[List[torch.Tensor]] = None,
    amp_level="O0",
    max_n_chunks: Optional[int] = None,
):
    print("Sampling")

    dtype = torch.float32 if amp_level not in ("O2", "O3") else torch.float16
    ...
    for k in c:
        if not k == "crossattn":
            c[k], uc[k] = map(
                lambda y: y[k][: int(torch.prod(torch.tensor(num_samples)))],
                (c, uc)
            )
    ...
    shape = (torch.prod(torch.tensor(num_samples)), C, H // F, W // F)
    randn = torch.randn(*shape, dtype=dtype)
